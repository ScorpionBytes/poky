...
class LoginTest(OERuntimeTestCase):
    def test_screenshot(self):
        ...
        with tempfile.NamedTemporaryFile(prefix="oeqa-screenshot-login", suffix=".png") as t:
            ...
            savedfile = "{0}/saved-{1}-core-image-sato-{2}.png".format(saved_screenshots_dir, \
                                                                        datetime.timestamp(datetime.now()), \
                                                                        self.td.get('MACHINE'))
            shutil.copy2(t.name, savedfile)

            refimage = self.td.get('COREBASE') + "/meta/files/image-tests/core-image-sato-" + self.td.get('MACHINE') +".png"
